...
const certificationsData: Certification[] = [
  {
    ...
    logo: ibmLogo, // Will be rendered with "SkillsBuild" label stacked
  ...
  },
  {
    ...
    issuer: 'IBM',
    logo: ibmLogo, // Will be rendered as standard horizontal IBM logo
...
function ProviderLogo({ logo, issuer }: { logo: any; issuer: string }) {
  ...
  // If there was an error loading, render initials fallback
  if (hasError) {
    return (
      <span className="text-xs font-bold text-zinc-400 select-none tracking-wider">
        {getInitials(issuer)}
      </span>
    );
  }

  // Case 1: Custom stacked rendering for IBM SkillsBuild
  if (issuer === 'IBM SkillsBuild') {
    return (
      <div className="flex flex-col items-center justify-center gap-0.5 w-full">
        ...
        />
        <span className="text-[7.5px] font-extrabold tracking-[0.05em] text-[#0F62FE] uppercase font-sans leading-none">
          SkillsBuild
        </span>
      </div>
    );
  ...
  // Case 2: Custom horizontal rendering for IBM
  if (issuer === 'IBM') {
    return (
      <img
        src={logo}
        alt="IBM"
        className="w-[50px] object-contain flex-shrink-0"
        onError={() => setHasError(true)}
      />
    );
  }

  // Case 3: Logo is a React Component (e.g. react-icons or lucide-react)
  if (typeof logo === 'function' || (typeof logo === 'object' && logo !== null)) {
    const Component = logo;
    return <Component className="w-[26px] h-[26px] object-contain flex-shrink-0" />;
  }

  // Case 4: Logo is a local path string (e.g. SVG / PNG imported url)
  ...
  // Final fallback if type mismatch
  return (
    <span className="text-xs font-bold text-zinc-400 select-none tracking-wider">
      {getInitials(issuer)}
    </span>
            ...
            >
              ...
              <div>
                ...
                <div className="flex items-center justify-between mb-5">
                  ...
                  {/* Logo Container: width is dynamically expanded for IBM to support horizontal logo */}
                  <div className={`${isIbm ? 'w-[72px]' : 'w-[42px]'} h-[42px] rounded-xl bg-zinc-900 border border-zinc-800 text-zinc-400 group-hover:text-white group-hover:bg-gradient-to-br group-hover:from-violet-600 group-hover:to-red-500 group-hover:border-transparent transition-all duration-300 flex items-center justify-center p-1.5 flex-shrink-0 shadow-inner select-none overflow-hidden`}>
                    <ProviderLogo logo={cert.logo} issuer={cert.issuer} />
                  </div>
                </div>
